...
class VideoImporter:
    ...
    def analyze(self) -> ImportReport:
        ...
        for video in videos:
            ...
            # Categorize for import - assign "Uncategorized" if no collection
            if not video.collection_name:
                video.collection_name = "Uncategorized"
                self.report.videos_without_collection += 1
    ...
    def update_collections_json(self, json_dir: Path) -> int:
        """Add any new collections (like Uncategorized) to collections.json and regenerate data.js.

        Returns the number of collections added.
        """
        collections_file = json_dir / "collections.json"
        data_js_file = json_dir / "data.js"
        ...
        added = 0
        for collection_name in self.report.by_collection.keys():
            # Check if collection already exists (by name)
            existing = any(c.get('name') == collection_name for c in collections.values())
            if not existing:
                # Generate a synthetic collection ID for imported collections
                collection_id = f"_imported_{self.sanitize_collection_name(collection_name).lower()}"
                collections[collection_id] = {
                    "id": collection_id,
                    "name": collection_name,
                    "total": len(self.report.by_collection[collection_name]),
                    "imported": True,
                }
                added += 1
                self.log(f"  Added collection: {collection_name}")
        ...
        if added > 0 and not self.dry_run:
            ...
            # Regenerate data.js
            content = "// Auto-generated by import_videos.py\n"
            content += f"window.collectionsData = {json.dumps(collections)};\n"
            content += f"window.videosData = {json.dumps(videos)};\n"
            with open(data_js_file, 'w', encoding='utf-8') as f:
                f.write(content)

            self.log(f"Updated collections.json and data.js with {added} new collections")
...
Examples:
  ...
  python import_videos.py --execute /source /dest
    ...
    parser.add_argument('--json-dir', type=str, metavar='DIR',
                        help='Directory containing collections.json and data.js (updates UI to show new collections)')
    ...
    if report.videos_to_import > 0:
        ...
        # Update collections.json and data.js for UI
        if args.json_dir:
            json_dir = Path(args.json_dir)
            print(f"\nUpdating collections.json in {json_dir}...")
            added = importer.update_collections_json(json_dir)
            if dry_run:
                print(f"[DRY RUN] Would add {added} new collections to collections.json")
            else:
                print(f"Added {added} new collections to collections.json and data.js")
