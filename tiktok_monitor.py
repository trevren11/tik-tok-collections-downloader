...
class DataStore:
    ...
    def _get_disk_only_collections(self) -> dict:
        ...
        existing_names = {c.get("name", "").lower() for c in self.collections.values()}

        # Scan disk for folders
        ...
        return disk_collections

    def _write_data_js(self):
        """Write data.js for the static viewer (allows viewing without a server)."""
        with self._save_lock:
            # Merge API collections with disk-only collections (like Uncategorized)
            all_collections = {**self.collections, **self._get_disk_only_collections()}

            content = "// Auto-generated by tiktok_monitor.py - do not edit\n"
            content += f"window.collectionsData = {json.dumps(all_collections)};\n"
...
class TikTokClient:
    ...
    def fetch_video_text(self, video_id: str) -> dict:
        """
        Fetch AI-generated text and metadata for a video using TikTok APIs.
        ...
        Returns dict with:
            ...
            - suggested_words: Related search terms
        """
        result = {
            "video_id": video_id,
            "ai_title": "",
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                logger.warning(f"Error fetching video text for {video_id}: {e}")
    ...
    def get_all_favorites(self, limit: Optional[int] = None) -> list:
        """Fetch ALL favorited/liked videos using direct API calls with pagination.

        This is more reliable than get_favorites() which uses scroll-based interception.
        Uses TikTok's favorite/item_list API endpoint with cursor-based pagination.
        """
        videos = []
        seen_ids = set()
        cursor = 0
        ...
        logger.info("Fetching all favorites via API...")
                ...
                while has_more and page_num < max_pages:
                    ...
                    try:
                        ...
                        for attempt in range(3):
                            response_data = page.evaluate("""
                                async ([cursor]) => {
                                    const url = new URL('https://www.tiktok.com/api/favorite/item_list/');
                                    url.searchParams.set('aid', '1988');
                                    url.searchParams.set('count', '30');
                                    url.searchParams.set('cursor', cursor.toString());
                                    url.searchParams.set('device_platform', 'web_pc');
                        ...
                        if not response_data or 'error' in response_data:
                            logger.warning(f"    [Favorites] API error after retries: {response_data.get('error', 'unknown') if response_data else 'no response'}")
                            break

                        status_code = response_data.get("status_code") or response_data.get("statusCode", -1)
                        if status_code != 0:
                            logger.warning(f"    [Favorites] API status {status_code}: {response_data.get('status_msg', 'unknown')}")
                            break
                        ...
                        if has_more:
                            ...
                            logger.info(f"    [Favorites] Fetched all {len(videos)} favorites")

                        if limit and len(videos) >= limit:
                            break

                    except Exception as e:
                        logger.warning(f"    [Favorites] Error fetching page {page_num}: {e}")
                        break

                    page.wait_for_timeout(500)

            except Exception as e:
                logger.warning(f"    [Favorites] Navigation error: {e}")
...
class VideoDownloader:
    ...
    def __init__(self, download_dir: str, cookies: dict = None, max_workers: int = 10, sessionid: str = None):
        self.download_dir = Path(download_dir)
        self.cookies = cookies or {}
        self.sessionid = sessionid or self.cookies.get("sessionid", "")
    ...
    def _metadata_needs_update(self, video_dir: Path) -> bool:
        """Check if metadata.json exists and has been processed for enriched fields."""
        metadata_file = video_dir / "metadata.json"
        ...
        try:
            ...
            # Check if we've already attempted to fetch metadata (flag set by _fetch_and_save_metadata)
            # This prevents re-fetching videos where TikTok simply doesn't provide AI content
            if metadata.get("metadata_fetched"):
                return False
            # Legacy check: if ai_article or subtitles are present and non-empty, consider it updated
            if metadata.get("ai_article") or metadata.get("subtitles"):
                return False
            # Also check if ai_title is set (some videos may not have article/subtitles)
            if metadata.get("ai_title"):
                return False
            return True
        except (json.JSONDecodeError, IOError):
            return True

    def _fetch_and_save_metadata(self, video_id: str, video_dir: Path, description: str, author: str):
        """
        Fetch AI-generated text and save enriched metadata.json.

        This includes:
        - Original description (title)
        - AI-generated title, article, description
        - Subtitles (speech-to-text transcript)
        - Stickers (text overlays)
        - Suggested search words
        """
        if not self.sessionid:
            # Can't fetch without session, just save basic metadata
            metadata = {
                "video_id": video_id,
                "author": author,
                "title": description,
                "original_desc": description,
                "metadata_fetched": True,  # Mark as processed even without session
            }
        ...
        try:
            # Fetch the AI-generated text using TikTok APIs
            with sync_playwright() as p:
                ...
                metadata = {
                    "video_id": video_id,
                    "author": author,
                    "title": description,  # Original short title
                    "original_desc": description,
                    "ai_title": "",
                    "ai_article": "",
                    "ai_desc": "",
                    ...
                    "stickers": [],
                    "suggested_words": [],
                    "metadata_fetched": True,  # Flag to indicate we attempted to fetch enriched metadata
                }

                # API 1: /api/customtdk/item/ - AI summary
                customtdk = page.evaluate("""
                    async (videoId) => {
                        const url = new URL('https://www.tiktok.com/api/customtdk/item/');
                        url.searchParams.set('itemId', videoId);
                ...
                if customtdk and "itemCustomTDK" in customtdk:
                    tdk = customtdk["itemCustomTDK"]
                    metadata["ai_title"] = tdk.get("title", "")
                    metadata["ai_article"] = tdk.get("article", "")
                    metadata["ai_desc"] = tdk.get("desc", "")
                    metadata["ai_keywords"] = tdk.get("keywords", [])

                # API 2: /api/item/detail/ - Details + subtitles
                detail = page.evaluate("""
                    async (videoId) => {
                        const url = new URL('https://www.tiktok.com/api/item/detail/');
                        url.searchParams.set('itemId', videoId);
                ...
                if detail and "itemInfo" in detail:
                    item = detail["itemInfo"].get("itemStruct", {})
                    # Use API desc if we don't have one
                    if not metadata["original_desc"]:
                        metadata["original_desc"] = item.get("desc", "")
                    metadata["suggested_words"] = item.get("suggestedWords", [])

                    # Stickers
                    for sticker in item.get("stickersOnItem", []):
                        text = sticker.get("stickerText", [])
                        if text:
                            metadata["stickers"].append(" ".join(text) if isinstance(text, list) else str(text))
                    ...
                    if subtitle_infos:
                        url = subtitle_infos[0].get("Url", "")
                        if url:
                            metadata["subtitles"] = fetch_subtitles(url)
            ...
            # Also save caption.txt with the best available text
            best_text = metadata.get("ai_article") or metadata.get("original_desc") or description
            if best_text:
                with open(video_dir / "caption.txt", "w") as f:
                    f.write(best_text)
    ...
    def download(self, video_id: str, author: str, collection_name: str, description: str = "") -> tuple[Optional[str], bool, Optional[str]]:
        ...
        if video_dir.exists():
            for f in video_dir.iterdir():
                if f.suffix.lower() in [".mp4", ".webm", ".mkv"]:
                    # File already exists - check if metadata needs updating
                    if self._metadata_needs_update(video_dir):
                        logger.debug(f"  Backfilling metadata for {video_id}")
                        self._fetch_and_save_metadata(video_id, video_dir, description, author)
            ...
            if result.returncode == 0:
                ...
                for ext in ["mp4", "webm", "mkv"]:
                    ...
                    if video_file.exists():
                        # Fetch and save enriched metadata (AI text, subtitles, etc.)
                        self._fetch_and_save_metadata(video_id, video_dir, description, author)
...
def cmd_sync(client: TikTokClient, store: DataStore, collection_limit: Optional[int] = None, video_limit: Optional[int] = None, downloader: Optional[VideoDownloader] = None, max_parallel: int = 3, exclude_collections: Optional[list] = None, full_sync: bool = False):
    """Sync collections and videos, queue new downloads. Optionally download after each collection.

    Args:
        full_sync: If True, always fetch all pages of each collection (no quick_check optimization)
    """
    logger.info("=== SYNC MODE ===" + (" (FULL)" if full_sync else ""))
    ...
    for i, coll in enumerate(collections_to_process, 1):
        ...
        try:
            if has_been_synced and not full_sync:
                ...
                # First time syncing this collection OR full_sync requested - fetch all
                logger.info(f"  [{i}/{len(collections_to_process)}] Fetching: {coll_name}" + (" (full sync)" if full_sync else ""))
                videos = client.get_collection_videos(coll_id, coll_name, limit=video_limit)
...
def cmd_watch(client: TikTokClient, store: DataStore, downloader: VideoDownloader, interval: int, exclude_collections: Optional[list] = None, collection_limit: Optional[int] = None, video_limit: Optional[int] = None, max_parallel: int = 3, full_sync: bool = False):
    ...
    """
    logger.info(f"=== WATCH MODE (every {interval} minutes)" + (" [FULL SYNC]" if full_sync else "") + " ===")
        ...
        while True:
            try:
                ...
                cmd_sync(client, store, collection_limit=collection_limit, video_limit=video_limit,
                        downloader=None, max_parallel=max_parallel, exclude_collections=exclude_collections, full_sync=full_sync)
...
def cmd_update_metadata(store: DataStore, downloader: VideoDownloader, limit: Optional[int] = None):
    """
    Scan all downloaded videos and backfill enriched metadata for any that don't have it.

    This is useful for updating metadata on videos downloaded before the metadata
    enrichment feature was added.
    """
    logger.info("=== UPDATE METADATA MODE ===")

    if not downloader.sessionid:
        logger.error("No sessionid available - cannot fetch metadata without authentication")
        return

    # Scan the download directory for all video folders
    download_dir = downloader.download_dir
    videos_to_update = []

    logger.info(f"Scanning {download_dir} for videos needing metadata updates...")
    ...
    for collection_folder in download_dir.iterdir():
        ...
        for video_folder in collection_folder.iterdir():
            if not video_folder.is_dir():
                continue

            video_id = video_folder.name

            # Check if this folder has a video file
            has_video = any(f.suffix.lower() in [".mp4", ".webm", ".mkv"] for f in video_folder.iterdir() if f.is_file())
            if not has_video:
                continue

            # Check if metadata needs updating
            if downloader._metadata_needs_update(video_folder):
                # Get video info from store if available
                video_data = store.videos.get(video_id, {})
    ...
    if total == 0:
        logger.info("All downloaded videos already have enriched metadata!")
        return

    logger.info(f"Found {total} videos needing metadata updates")

    if limit:
        videos_to_update = videos_to_update[:limit]
        logger.info(f"Processing {len(videos_to_update)} videos (limited)")
    ...
    for i, video in enumerate(videos_to_update, 1):
        ...
        logger.info(f"  [{i}/{len(videos_to_update)}] Updating metadata for {video_id} ({collection})")

        try:
            downloader._fetch_and_save_metadata(video_id, video_dir, desc, author)
            updated += 1
        except Exception as e:
            logger.warning(f"    Failed: {e}")
            failed += 1

    logger.info("=== UPDATE METADATA COMPLETE ===")
    logger.info(f"Updated: {updated}")
    logger.info(f"Failed: {failed}")
    logger.info(f"Remaining: {total - len(videos_to_update)}")


def cmd_fetch_all_favorites(client: TikTokClient, store: DataStore, downloader: VideoDownloader, max_parallel: int = 3):
    """
    Fetch ALL favorites using direct API pagination and queue for download.

    This uses the API-based get_all_favorites() method which is more reliable
    than scroll-based interception and can fetch the complete favorites list.
    """
    logger.info("=== FETCH ALL FAVORITES MODE ===")

    # Fetch all favorites
    logger.info("Fetching ALL favorited videos (this may take a while)...")
    favorites = client.get_all_favorites()
    logger.info(f"Found {len(favorites)} total favorited videos")
    ...
    for vid in favorites:
        ...
        # Skip if this video is already in a collection (not favorites)
        if vid_id in store.videos and store.videos[vid_id].get("collection_id") != "_favorites":
            continue
    ...
    # Check for favorites that are now missing (unfavorited)
    deleted_count = 0
    for vid_id, vid_data in store.videos.items():
        if vid_data.get("collection_id") == "_favorites":
            if vid_id not in found_favorite_ids and not vid_data.get("deleted_from_tiktok"):
                ...
                vid_data["deleted_at"] = datetime.now().isoformat()
                deleted_count += 1
                logger.info(f"  Marked favorite as removed: {vid_id}")

    logger.info(f"New favorites: {new_count}, queued: {queued_count}, removed: {deleted_count}")

    store.save_videos()
    store.save_queue()

    # Download queued videos
    if queued_count > 0:
        logger.info("Downloading new favorites...")
        downloaded = _download_collection_queue(store, downloader, "Favorites", max_parallel)
        logger.info(f"Downloaded: {downloaded} favorites")

    logger.info("=== FETCH ALL FAVORITES COMPLETE ===")
...
def main():
    ...
    parser.add_argument("--update-metadata", action="store_true", help="Backfill enriched metadata for all downloaded videos")
    parser.add_argument("--full-sync", action="store_true", help="Force full sync of all collections (fetch all pages, not just recent)")
    parser.add_argument("--fetch-all-favorites", action="store_true", help="Scroll through ALL favorites (may take a while)")
    ...
    downloader = VideoDownloader(download_dir, cookies, sessionid=sessionid)
    ...
    if args.status:
        ...
        cmd_update_metadata(store, downloader, download_limit)
    elif args.fetch_all_favorites:
        cmd_fetch_all_favorites(client, store, downloader, max_parallel=max_parallel)
    elif args.sync:
        cmd_sync(client, store, collection_limit, video_limit, exclude_collections=exclude_collections, max_parallel=max_parallel, full_sync=args.full_sync)
    elif args.download:
        cmd_download(store, downloader, download_limit, max_parallel=max_parallel)
    elif args.watch:
        cmd_watch(client, store, downloader, args.interval, exclude_collections=exclude_collections, collection_limit=collection_limit, video_limit=video_limit, max_parallel=max_parallel, full_sync=args.full_sync)
    else:
